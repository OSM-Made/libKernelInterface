...
class SignedElfNpdrmControlBlock(object):
	...
	def save(self, f):
		f.write(struct.pack(
            SignedElfNpdrmControlBlock.FMT,
            int(self.type),
            self.content_id.encode('utf-8'),  # Encode string to bytes
            self.random_pad.encode('utf-8')  # Encode string to bytes
        ))
...
class SignedElfFile(object):
	...
	def save(self, f):
		...
		f.write(struct.pack(
            ...
            self.magic.encode('utf-8'),  # Encode string to bytes
            int(self.version),
            int(self.mode),
            int(self.endian),
            int(self.attribs)
		...
		for entry in self.entries:
			...
			if isinstance(entry.data, str):
				f.write(entry.data.encode('utf-8'))
			else:
				f.write(entry.data)
